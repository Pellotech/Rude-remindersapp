...
const RUDY_LINES = {
  ...
  manageLoad: {
    rude: [
      ...
      "Tap the green or red emoji. Don't pretend you forgot.",
      "Those emojis aren't decorations. Use them.",
      "Your track record is right here. Scary, isn't it?",
      "Log it. Green or red. No hiding.",
      "I see everything on this tab. Everything.",
    ],
    positive: [
      "Your history is right here. Own every entry.",
      "Tap 😊 if you did it, 😔 if you didn't. Simple.",
      "Every entry matters. Tap the emoji to log it.",
      "Every entry is data. Use it to get better.",
      "This is where habits are built — one honest tap at a time.",
    ],
  },
  manageOverdue: {
    rude: [
...
export default function RudyWidget({ nudgeEvent, nudgeKey, onNudgeHandled, showReactionBubble, showPremiumButton, onPremiumPress }: RudyWidgetProps) {
  ...
  // ─── Tap handler ───────────────────────────────────────────────────────────
  function handleTap() {
    fireReaction("You gave me a fist bump, now let's get things done.", RUDY_IMAGES.fistBumpStandingSmirk, false);
  }
