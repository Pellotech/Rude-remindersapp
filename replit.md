...
- **Subscription System**: Differentiates features for free and premium users, including AI-generated content.
  ...
  - **6-Month Free Tier Step-Down**: After `FREE_TIER_DURATION_MONTHS = 6` months past `users.createdAt`, free users' monthly limit drops from `FREE_MONTHLY_LIMIT = 15` to `POST_TRIAL_FREE_MONTHLY_LIMIT = 10` (no hard block — they can keep using the app). Helper `getFreeLimitForUser(createdAt)` in `server/utils/premiumCheck.ts` is called by both `checkMonthlyReminderLimit` and `atomicIncrementAndCheck`. Whitelisted accounts and active premium subscribers bypass via existing `isUserPremium()`. UTC-safe expiry math via `getFreeTierExpiryDate()` / `isFreeTrialExpired()`.
