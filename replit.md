...
- **Subscription System**: Differentiates features for free and premium users, including AI-generated content.
  ...
  - **6-Month Free Tier Expiration**: Free accounts expire `FREE_TIER_DURATION_MONTHS = 6` months after `users.createdAt`. After expiry, reminder creation is blocked with HTTP 402 + `code: 'FREE_TRIAL_EXPIRED'` and a paywall message; user can still sign in and view existing data. Helpers: `getFreeTierExpiryDate(createdAt)`, `isFreeTrialExpired(createdAt)`. Both `checkMonthlyReminderLimit` and `atomicIncrementAndCheck` return `trialExpired: true` when triggered. Whitelisted accounts (e.g. `appstoreuser@rudereminders.com`) and active premium subscribers bypass via existing `isUserPremium()`. `/api/auth/user` exposes `freeTierExpiresAt` (ISO) and `freeTrialExpired` (boolean) for client paywall UI.
